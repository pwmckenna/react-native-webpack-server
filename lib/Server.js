...
class Server {
   ...
   * @param {Number}  port (default 8081)
   * @param {Number}  packagerPort (default 8082)
   * @param {Number}  webpackPort (default 8083)
   * @param {Array}   entries (default [index.ios, index.android])
   * @param {String}  androidEntry (default index.android)
  ...
  constructor(options) {
    ...
    this.packagerPort = options.packagerPort || 8082;
    this.webpackPort = options.webpackPort || 8083;
    this.entries = options.entries || ['index.ios', 'index.android'];
    ...
    this.reactCodeURLs = _.object(this.entries, this.entries.map(entry =>
      url.resolve(this.packagerBaseURL, `${entry}.bundle`)
    ));
    this.reactMapURLs = _.object(this.entries, this.entries.map(entry =>
      url.resolve(this.packagerBaseURL, `${entry}.map`)
    ));
    this.appCodeURLs = _.object(this.entries, this.entries.map(entry =>
      url.resolve(this.webpackBaseURL, `${entry}.js`)
    ));
    this.appMapURLs = _.object(this.entries, this.entries.map(entry =>
      url.resolve(this.webpackBaseURL, `${entry}.js.map`)
    ));
    ...
    this._readyPromise = this._startWebpackDevServer().then(() =>
  ...
  start() {
    ...
    this.httpServer = http.createServer(this.server).listen(port, () => {
  ...
  handleRequest(req, res, next) {
    ...
    const entry = this.entries.find(e => 
      pathname === `/${e}.bundle` || pathname === `/${e}.map`
    );
    if (!entry) {
      res.writeHead(404);
  ...
  handleBundleRequest(req, res, next) {
    ...
    let reactCodeURL = this.reactCodeURLs[req.entry];
    let appCodeURL = this.appCodeURLs[req.entry];
    if (urlSearch) {
    ...
    Promise.props({
      ...
      createBundleCode(r.reactCode, r.appCode, req.entry)
    ).then(bundleCode => {
      res.writeHead(200);
      res.end(bundleCode);
    }).catch(next);
  ...
  handleMapRequest(req, res, next) {
    ...
    // Forward URL params to RN packager
    let reactCodeURL = this.reactCodeURLs[req.entry];
    let reactMapURL = this.reactMapURLs[req.entry];
    let appCodeURL = this.appCodeURLs[req.entry];
    let appMapURL = this.appMapURLs[req.entry];
    if (urlSearch) {
      reactCodeURL += urlSearch;
  ...
  _createBundleCode(reactCode, appCode, entry) {
    reactCode = reactCode.replace(SOURCEMAP_REGEX, '');
    appCode = appCode.replace(SOURCEMAP_REGEX, '');
    return reactCode + appCode + '//# sourceMappingURL=/' + entry + '.map';
  }
  ...
  _writeEntryFiles() {
    ...
    this.entries.forEach(entry => {
      const entryFile = path.join(this.entryDir, entry + '.js');
  ...
  _startWebpackDevServer() {
    ...
    return getReactNativeExternals().then(reactNativeExternals => {
      return new Promise(resolve => {
        ...
        this.webpackServerHttp = this.webpackServer.listen(this.webpackPort, this.hostname, () => {
